...
// 이벤트"(현재 반경 기준)에 뜬 이벤트가 지도 페이지 하단 목록엔 안 뜨는 불일치가 생길 수
...
async function initMapPageMap(focusEvent) {
  ...
  try {
    ...
    if (!mapPageInitialized) {
      ...
      // "내 주변 인기 이벤트"(홈)와 정확히 같은 반경이 지도 화면에 담기도록 맞춘다
      // — 홈에서 보이던 이벤트가 지도 하단 목록에도 그대로 나오게 하기 위함.
      if (!focusEvent) fitMapToRadiusKm(loc, 10);
      ...
      if (focusEvent) {
        ...
        const isValidCoord = Number.isFinite(loc.lat) && Number.isFinite(loc.lng)
          && Math.abs(loc.lat) <= 90 && Math.abs(loc.lng) <= 180;
        if (isValidCoord) {
          fitMapToRadiusKm(loc, 10); // "내 주변 인기 이벤트"와 동일한 10km 반경으로 정확히 맞춤
