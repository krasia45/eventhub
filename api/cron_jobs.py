...
5. ⚠️ 출처 우선순위를 반드시 지켜: 1순위는 브랜드 공식 홈페이지, 브랜드 공식 인스타그램/블로그,
   브랜드 공식 앱 공지사항이야. 이게 있으면 반드시 이걸 출처로 써. 뉴스 기사는 브랜드 공식 채널을
   아무리 찾아도 못 찾았을 때만 차선책으로 써 — 뉴스는 2차 정보라 보도 시점 이후 기간이 연장/조기
   종료/취소됐을 수 있고 오탈자·부정확한 요약도 섞여있어. "쿠폰 총정리", "할인코드 모음" 같은
   제휴 마케팅/블로그성 사이트는 절대 출처로 쓰지 마.
